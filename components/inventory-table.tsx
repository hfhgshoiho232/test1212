...
import { inventoryData, Part } from '@/lib/inventory-data'
...
export default function InventoryTable({ manufacturer, searchTerm }: InventoryTableProps) {
  const [currentPage, setCurrentPage] = useState(1)
  const [itemsPerPage, setItemsPerPage] = useState(25)

  const filteredParts = inventoryData.filter(part => 
    part.manufacturer === manufacturer.toUpperCase() &&
    part.partNumber.toLowerCase().includes(searchTerm.toLowerCase())
  )

  const totalPages = Math.ceil(filteredParts.length / itemsPerPage)
  const startIndex = (currentPage - 1) * itemsPerPage
  const visibleParts = filteredParts.slice(startIndex, startIndex + itemsPerPage)
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        ...
        <div>
          Showing {startIndex + 1} to {Math.min(startIndex + itemsPerPage, filteredParts.length)} of {filteredParts.length} entries
        </div>
      </div>
            ...
            {visibleParts.map((part, index) => (
              <TableRow key={part.id} className={index % 2 === 0 ? 'bg-gray-50' : 'bg-white'}>
                ...
                <TableCell className="text-right">
                  <Link 
                    href={`/inventory/${manufacturer.toLowerCase()}/${part.partNumber.toLowerCase().replace(/\s+/g, '-')}`}
